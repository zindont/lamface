...
class scssc
{
    ...
    protected function reduce($value, $inExp = false)
    {
        ...
        switch ($type) {
            case "exp":
                ...
                // this tries:
                // 1. scss_[op name]_[left type]_[right type]
                // 2. scss_[left type]_[right type] (passing the op as first arg
                // 3. scss_[op name]
                $fn = "scss_${opName}_${ltype}_${rtype}";
                if (is_callable(array($this, $fn)) || (($fn = "scss_${ltype}_${rtype}") && is_callable(array($this, $fn)) && $passOp = true) || (($fn = "scss_${opName}") && is_callable(array($this, $fn)) && $genOp = true)) {
                    $unitChange = false;
    ...
    protected function scss_add_number_number($left, $right)
    {
        return array("number", $left[ 1 ] + $right[ 1 ], $left[ 2 ]);
    }

    protected function scss_mul_number_number($left, $right)
    {
        return array("number", $left[ 1 ] * $right[ 1 ], $left[ 2 ]);
    }

    protected function scss_sub_number_number($left, $right)
    {
        return array("number", $left[ 1 ] - $right[ 1 ], $left[ 2 ]);
    }

    protected function scss_div_number_number($left, $right)
    {
        return array("number", $left[ 1 ] / $right[ 1 ], $left[ 2 ]);
    }

    protected function scss_mod_number_number($left, $right)
    {
        return array("number", $left[ 1 ] % $right[ 1 ], $left[ 2 ]);
    }

    // adding strings
    protected function scss_add($left, $right)
    {
    ...
    protected function scss_and($left, $right, $shouldEval)
    {
        if (!$shouldEval) {
            return;
        }
    ...
    protected function scss_or($left, $right, $shouldEval)
    {
        if (!$shouldEval) {
            return;
        }
    ...
    protected function scss_color_color($op, $left, $right)
    {
        $out = array('color');
        foreach (range(1, 3) as $i) {
            ...
            switch ($op) {
                ...
                case '/':
                    ...
                    break;
                case "==":
                    return $this->scss_eq($left, $right);
                case "!=":
                    return $this->scss_neq($left, $right);
                default:
    ...
    protected function scss_color_number($op, $left, $right)
    {
        $value = $right[ 1 ];

        return $this->scss_color_color($op, $left, array("color", $value, $value, $value));
    }

    protected function scss_number_color($op, $left, $right)
    {
        $value = $left[ 1 ];

        return $this->scss_color_color($op, array("color", $value, $value, $value), $right);
    }

    protected function scss_eq($left, $right)
    {
    ...
    protected function scss_neq($left, $right)
    {
        return $this->toBool($left != $right);
    }

    protected function scss_gte_number_number($left, $right)
    {
        return $this->toBool($left[ 1 ] >= $right[ 1 ]);
    }

    protected function scss_gt_number_number($left, $right)
    {
        return $this->toBool($left[ 1 ] > $right[ 1 ]);
    }

    protected function scss_lte_number_number($left, $right)
    {
        return $this->toBool($left[ 1 ] <= $right[ 1 ]);
    }

    protected function scss_lt_number_number($left, $right)
    {
        return $this->toBool($left[ 1 ] < $right[ 1 ]);
    }
    ...
    {
        if (!isset($env) || !empty($env->block->type) && $env->block->type != "media") {
            return $childQueries;
        }
        ...
        switch ($value[ 0 ]) {
            ...
            case "keyword":
                ...
                if (isset(self::$cssColors[ $name ])) {
                    ...
                    return isset($rgba[ 3 ]) ? array('color', (int)$rgba[ 0 ], (int)$rgba[ 1 ], (int)$rgba[ 2 ], (int)$rgba[ 3 ]) : array('color', (int)$rgba[ 0 ], (int)$rgba[ 1 ], (int)$rgba[ 2 ]);
                }
    ...
    protected static $lib_hsla = array(
        "hue",
        "saturation",
        "lightness",
        "alpha"
...
class scss_parser
{
    ...
    protected function parseChunk()
    {
        ...
        if (isset($this->buffer[ $this->count ]) && $this->buffer[ $this->count ] == "@") {
            ...
            if ($this->literal("@extend") && $this->selectors($selector) && $this->end()) {
            ...
            if ($this->literal("@each") && $this->variable($varName) && $this->literal("in") && $this->valueList($list) && $this->literal("{")) {
            ...
            if (($this->literal("@debug") || $this->literal("@warn")) && $this->valueList($value) && $this->end()) {
            ...
            if ($this->literal("@", false) && $this->keyword($dirName) && ($this->openString("{", $dirValue) || true) && $this->literal("{")) {
...
class scss_formatter
{
    ...
    protected function block($block)
    {
        ...
        if (!empty($block->selectors)) {
            echo '' . $pre . implode($this->tagSeparator, $block->selectors) . $this->open . $this->break;
            $this->indentLevel++;
...
class scss_server
{
    ...
    protected function compile($in, $out)
    {
        ...
        $wp_filesystem->put_contents($this->importsCacheName($out), serialize($this->scss->getParsedFiles()));
    ...
    public function __construct($dir, $cacheDir = null, $scss = null)
    {
        ...
        if (!is_dir($this->cacheDir)) {
            wp_mkdir_p($this->cacheDir, 0755, true);
        }
